...
const littleEndian = (function (): boolean {
 ...
 * Handles decompression of JPEG lossless encoded images
 */
export class Decoder {
  /** IDCT permutation table */
  static readonly IDCT_P: readonly number[] = [
  ...
  static readonly MAX_HUFFMAN_SUBTREE = 50;
  static readonly MSB = 0x80000000;
  static readonly RESTART_MARKER_BEGIN = 0xffd0;
  static readonly RESTART_MARKER_END = 0xffd7;

  buffer: ArrayBufferLike | null = null;
  stream: DataStream | null = null;
  frame: FrameHeader = new FrameHeader();
  huffTable: HuffmanTable = new HuffmanTable();
  quantTable: QuantizationTable = new QuantizationTable();
  scan: ScanHeader = new ScanHeader();

  /** Data units - at most 10 data units in a MCU, at most 4 data units in one component */
  DU: number[][][] = createArray<number[][]>(10, 4, 64);

  /** Huffman lookup tables [4][2][50*256] */
  HuffTab: number[][][] = createArray<number[][]>(4, 2, 50 * 256);

  IDCT_Source: number[] = [];

  /** Number of blocks in the i-th component in a scan */
  nBlock: number[] = [];

  /** AC HuffTab for the i-th component in a scan */
  acTab: number[][] = createArray<number[]>(10, 1);

  /** DC HuffTab for the i-th component in a scan */
  dcTab: number[][] = createArray<number[]>(10, 1);

  /** Quantization table for the i-th component in a scan */
  qTab: number[][] = createArray<number[]>(10, 1);

  marker: number = 0;
  markerIndex: number = 0;
  numComp: number = 0;
  restartInterval: number = 0;
  selection: number = 0;
  xDim: number = 0;
  yDim: number = 0;
  xLoc: number = 0;
  yLoc: number = 0;
  outputData: OutputData | null = null;
  ...
  getter: GetterFunction | null = null;
  setter: SetterFunction | null = null;
  output: OutputFunction | null = null;
  selector: SelectorFunction | null = null;

  /**
   * The Decoder constructor
   * @param buffer - Optional input buffer
   * @param numBytes - Number of bytes per component
   */
  constructor(buffer?: ArrayBufferLike, numBytes?: number) {
   ...
   * @param buffer - Input JPEG buffer
   * @param offset - Byte offset to start reading
   * @param length - Number of bytes to read
   * @returns Decompressed pixel data buffer
   */
  decompress(
  ...
  decode(
    ...
    while ((current >> 4) !== 0xffc || current === 0xffc4) {
      // DHT marker
      switch (current) {
        case 0xffc4: // DHT
    ...
    do {
      ...
      if (this.numBytes === 1) {
        ...
        this.outputData = new Uint16Array(
          new ArrayBuffer(
            this.xDim * this.yDim * this.numBytes * this.numComp * 2
          )
        );
      }
      ...
      while (true) {
        ...
        if (
          !(
            current >= Decoder.RESTART_MARKER_BEGIN &&
            current <= Decoder.RESTART_MARKER_END
          )
        ) {
          break;
        }
  ...
  select7 = (compOffset?: number): number => {
    return (
      (this.getPreviousX(compOffset) + this.getPreviousY(compOffset)) / 2
    );
  };
      ...
      for (i = 0; i < this.nBlock[ctrC]; i += 1) {
        ...
        for (j = 1; j < 64; j += 1) {
          ...
          if ((value & 0x0f) === 0) {
            ...
            this.IDCT_Source[Decoder.IDCT_P[j]] =
  ...
  decodeSingle(prev: number[], temp: number[], index: number[]): number {
    ...
    for (i = 0; i < this.nBlock[0]; i += 1) {
      ...
      if (
        nRestart >= Decoder.RESTART_MARKER_BEGIN &&
        nRestart <= Decoder.RESTART_MARKER_END
      ) {
        return nRestart;
      }
  ...
  /**
   * Get Huffman value from table
   *
   * HuffmanValue(table HuffTab[x][y]):
   *   return: Huffman Value of table
   *           0xFF?? if it receives a MARKER
   *   Parameter: table HuffTab[x][y] (ex) HuffmanValue(HuffTab[1][0],...)
   *              temp temp storage for remained bits
  ...
  getPreviousX(compOffset: number = 0): number {
  ...
  getPreviousXY(compOffset: number = 0): number {
    if (this.getter === null) throw new Error("decode hasn't run yet");
  ...
  getPreviousY(compOffset: number = 0): number {
    if (this.getter === null) throw new Error("decode hasn't run yet");
  ...
  setValueRGB = (index: number, val: number, compOffset: number = 0): void => {
  ...
  getValueRGB = (index: number, compOffset?: number): number => {
    ...
    return this.outputData[index * 3 + (compOffset ?? 0)];
  };
